...
export default function DashboardPage({ products, orders, loading }) {
  ...
  // Logic for products
  const mostSoldCount = useMemo(() => safeProducts.filter(p => (p.reviewsCount || 0) > 0).length, [safeProducts]);
  const leastSoldCount = useMemo(() => safeProducts.filter(p => (p.reviewsCount || 0) === 0).length, [safeProducts]);
  const cancelledCount = useMemo(() => safeOrders.filter(o => o && (o.status === "Cancelled" || o.status === "Returned")).length, [safeOrders]);
  ...
  return (
    <div className="flex flex-col gap-6 w-full">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-6">
        ...
        {loading || fetching ? <Skeleton w="100%" h={220} r={12} /> : (
          <div className="w-full h-[220px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={analytics.trend || []} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
                <defs>
