...
export default function ReviewsPage() {
  ...
  // const [showBulkActions, setShowBulkActions] = useState(false); // Removed unused variable
  ...
  const fetchReviews = useCallback(async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/reviews/seller/${currentUser.id}`);
      if (res.ok) {
  ...
  const aiInsights = useMemo(() => {
    if (reviews.length === 0) return { sentiment: "neutral", issues: [] };
    
    const lowRated = reviews.filter(r => r.rating <= 3);
    const total = lowRated.length || 1;
    
    return {
      sentiment: reviews.filter(r => r.rating >= 4).length > reviews.length / 2 ? "positive" : "needs improvement",
      issues: [
        { label: "Delivery Issues", percentage: Math.round((lowRated.filter(r => /delivery|delay|time/i.test(r.body || r.title)).length / total) * 100), color: "#ef4444" },
        { label: "Quality/Packaging", percentage: Math.round((lowRated.filter(r => /quality|pack|damage/i.test(r.body || r.title)).length / total) * 100), color: "#f59e0b" },
        { label: "Missing/Wrong Items", percentage: Math.round((lowRated.filter(r => /missing|wrong|item/i.test(r.body || r.title)).length / total) * 100), color: "#3b82f6" }
  ...
  */
  ...
  const filteredReviews = reviews
    ...
    .filter(r => {
      ...
      if (searchTerm && !(r.body || "").toLowerCase().includes(searchTerm.toLowerCase()) && !r.title?.toLowerCase().includes(searchTerm.toLowerCase())) return false;
      return true;
    })
    .sort((a, b) => {
  ...
  return (
    <div className="flex flex-col gap-5 w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 mt-2">
        {loading ? <Skeleton w="100%" h={200} r={16} count={2} /> : filteredReviews.length === 0 ? (
          ...
          filteredReviews.map(r => {
            const isSelected = selectedReviews.includes(r.review_id);
            return (
              <div key={r.review_id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl relative shadow-sm hover:shadow-md transition-shadow">
                <input type="checkbox" checked={isSelected} onChange={() => selectReview(r.review_id)} className="absolute top-5 left-4 w-4 h-4 cursor-pointer text-emerald-500 rounded" />
                <div className="p-5 pl-12 flex flex-col h-full">
                  <div className="flex justify-between items-center mb-3">
                    <span className="text-[12px] font-bold text-slate-400">Order-{r.order_id || 'N/A'}</span>
                    <div className="flex items-center gap-2">
                       <StarRating rating={r.rating} size={14} />
                       <span className="text-[11px] font-medium text-slate-500">{new Date(r.created_at).toLocaleDateString()}</span>
                    </div>
                  </div>
                  
                  <div className="mb-3">
                    <span className="text-[11px] font-medium text-slate-500">Item:</span>
                    <div className="text-[13px] font-bold text-emerald-600 dark:text-emerald-500 mt-0.5">{r.product_name}</div>
                  </div>
                  
                  <div className="mb-2 font-bold text-[14px] text-slate-800 dark:text-slate-100">{r.title}</div>
                  <p className="text-[13px] text-slate-700 dark:text-slate-300 leading-relaxed mb-4 flex-1">
                    {(r.body || "").length > 150 ? `${(r.body || "").substring(0, 150)}...` : (r.body || "")}
                  </p>
                  
                  <div className="flex justify-between items-end gap-3 mt-auto pt-4 border-t border-slate-100 dark:border-slate-800">
                    <div className="flex items-center gap-2.5">
                      <div className={`w-9 h-9 rounded-full flex items-center justify-center text-[12px] font-bold text-white ${avatarBg(r.customer_name || "User")}`}>
                        {(r.customer_name || "U").split(" ").map(w => w[0]).join("")}
                      </div>
                      <div>
                        <div className="text-[12px] font-bold text-slate-800 dark:text-slate-200">{r.customer_name}</div>
                        <span className="text-[10px] font-bold text-emerald-500 mt-0.5">Verified Purchase</span>
                      </div>
                    </div>
                  </div>
                </div>
