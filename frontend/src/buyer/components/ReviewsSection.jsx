...
const ReviewsSection = ({ product }) => {
  ...
  const fetchReviews = async () => {
    try {
      const res = await fetch(`http://localhost:5000/api/reviews/product/${product.id}`);
      if (res.ok) {
        const data = await res.json();
        const mapped = data.map(r => ({
          id: r.review_id,
          user: r.customer_name || "User",
          rating: r.rating,
          date: new Date(r.created_at).toLocaleDateString(),
          title: r.title,
          comment: r.body,
          images: [], // Images not yet implemented in backend schema properly
          videos: [],
          helpful: 0
        }));
        setReviews(mapped);
              ...
              <div className="form-group">
                ...
                <div className="flex flex-col gap-4 bg-slate-50 p-5 rounded-2xl border border-slate-200">
                  <div className="flex gap-4">
                    <button 
                      type="button" 
                      className={`flex-1 py-2.5 rounded-lg border font-bold text-[0.9rem] cursor-pointer transition-all duration-300 ${mediaType === 'image' ? 'bg-[#007185] text-white border-[#007185] shadow-sm' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-300'}`} 
                    ...
                    <button 
                      type="button" 
                      className={`flex-1 py-2.5 rounded-lg border font-bold text-[0.9rem] cursor-pointer transition-all duration-300 ${mediaType === 'video' ? 'bg-[#007185] text-white border-[#007185] shadow-sm' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-300'}`} 
