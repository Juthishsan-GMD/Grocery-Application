  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.removeItem('fb_onboarding_done'); // Force clear any stale flag before loginUser sets it
        loginUser(data.user);
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-5 flex flex-col gap-2">
                ...
                <div className="relative flex items-center">
                  ...
                  <input type="email" placeholder="name@example.com" required 
                    className="w-full py-4 pr-5 pl-14 border-2 border-gray-200 rounded-xl bg-white text-base transition-all duration-300 focus:border-[#10b981] focus:shadow-[0_4px_15px_rgba(16,185,129,0.1)] outline-none text-gray-900"
                    value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} disabled={otpSent} />
                </div>
              </div>

              <div className="mb-5 flex flex-col gap-2">
                <label className="text-[0.9rem] text-gray-900 font-semibold">OTP Verification</label>
                <div className="flex gap-2">
                  <input type="text" placeholder="Enter 6-digit OTP" required={isSignUp}
                    className="flex-1 py-4 px-5 border-2 border-gray-200 rounded-xl bg-white text-base transition-all duration-300 focus:border-[#10b981] focus:shadow-[0_4px_15px_rgba(16,185,129,0.1)] outline-none text-gray-900"
                    value={formData.otp} onChange={(e) => setFormData({...formData, otp: e.target.value})} disabled={!otpSent} />
                  <button type="button" onClick={handleSendOtp} disabled={otpLoading}
                    className="px-6 py-4 rounded-xl text-[0.95rem] font-bold bg-emerald-100 text-emerald-700 hover:bg-emerald-200 transition-colors border-none cursor-pointer whitespace-nowrap">
