...
import { FiArrowLeft, FiShield, FiMail, FiLock, FiUser } from "react-icons/fi";
...
export default function AdminSignupPage() {
  ...
  const handleSendOtp = async () => {
    if (!formData.email) {
      showToast("Please enter official email first.");
      return;
    }
    setOtpLoading(true);
    try {
      const resp = await fetch("http://localhost:5000/api/auth/send-otp", {
        ...
        body: JSON.stringify({ email: formData.email, role: "admin" })
      });
      const data = await resp.json();
      if (resp.ok) {
        showToast("OTP sent to official email!");
        setOtpSent(true);
      } else {
      ...
      <div className="admin-right-form">
        <div style={{ maxWidth: "450px", margin: "0 auto", width: "100%" }}>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="admin-field-group">
              <label style={{ fontSize: "0.85rem", fontWeight: 600, color: G.muted }}>OTP VERIFICATION</label>
              <div style={{ display: "flex", gap: "10px" }}>
                <div className="admin-input-box" style={{ flex: 1 }}>
                  <FiLock />
                ...
                <button type="button" onClick={handleSendOtp} disabled={otpLoading} style={{ padding: "0 1.5rem", borderRadius: "10px", border: "none", background: "#e0e7ff", color: G.primary, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>
                  {otpLoading ? "Sending..." : (otpSent ? "Resend" : "Send OTP")}
                </button>
              </div>
            </div>            <div className="admin-field-group">
