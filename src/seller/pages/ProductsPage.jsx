...
import { PageHeader, Skeleton, BackButton, SectionHeader } from '../components/sharedComponents';
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl p-6 border border-slate-200 dark:border-slate-800 shadow-sm relative">
              ...
              <div className="flex flex-col gap-5">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                     <label className="text-[14px] font-semibold mb-2 block text-slate-800 dark:text-slate-200">Category *</label>
                     <select name="category" value={form.category} onChange={handleCategoryChange} className={selectClass}>
                       {CATEGORY_OPTIONS.map((c) => <option key={c} value={c}>{c}</option>)}
                     </select>
                  </div>
                  <div>
                     <label className="text-[14px] font-semibold mb-2 block text-slate-800 dark:text-slate-200">Subcategory *</label>
          ...
          <div className="flex flex-col gap-6 sticky top-6 md:col-span-1">
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
              ...
              <div className="p-6 flex justify-center bg-slate-50 dark:bg-slate-900/50">
                <div className="w-full max-w-[240px] bg-white dark:bg-[#313131] rounded-[20px] shadow-[0_10px_30px_rgba(0,0,0,0.1)] dark:shadow-black/40 overflow-hidden border border-slate-100 dark:border-slate-700 transition-all duration-300">
                  ...
                  <div className="p-4 flex flex-col gap-2">
                    ...
                    <h4 className="font-bold text-[14px] m-0 overflow-hidden text-ellipsis whitespace-nowrap text-slate-800 dark:text-slate-100">{form.name || "Product Title"}</h4>
                    ...
                    <button className="w-full h-9 rounded-xl bg-emerald-500/10 text-emerald-600 font-bold text-[12px] border-none cursor-default mt-1">
                      Add To Cart
                    </button>
                  </div>
                </div>
  ...
  const saveProduct = async (productData) => {
    try {
      if (editProduct) {
        ...
        toast({ title: result?.ok ? "Product Published" : "Publish Failed", description: result?.ok ? "Your new product is live." : "Could not save the new product.", variant: result?.ok ? "default" : "destructive" });
      }
      setEditorView(false);
      setEditProduct(null);
    } catch (err) {
